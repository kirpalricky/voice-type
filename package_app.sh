#!/bin/bash
# Builds Yapboard in release mode and assembles a self-contained .app bundle.
...
# clobber the same Yapboard.<VERSION>.app on every run.
BUILD=$(( $(/usr/libexec/PlistBuddy -c "Print :CFBundleVersion" "$PLIST") + 1 ))
/usr/libexec/PlistBuddy -c "Set :CFBundleVersion $BUILD" "$PLIST"

APP="$OUT_DIR/Yapboard.$VERSION.$BUILD.app"
...
cp "$ARCH_DIR/Yapboard" "$APP/Contents/MacOS/Yapboard"
cp Sources/Yapboard/Resources/Info.plist "$APP/Contents/Info.plist"
cp Sources/Yapboard/Resources/AppIcon.icns "$APP/Contents/Resources/AppIcon.icns"
...
if [[ -d "$APP/Contents/Frameworks/Sparkle.framework" ]]; then
    ...
    install_name_tool -add_rpath @executable_path/../Frameworks "$APP/Contents/MacOS/Yapboard"
fi
